...
Apx.prototype.start = function(){
  ...
  self.sysLog.info('Setting up log transports')
  //setup logging
  var logTransports = []
  if(self.config.get('winston.console')){
    logTransports.push(new (winston.transports.Console)())
  }
  if(self.config.get('winston.file')){
    logTransports.push(new (winston.transports.File)({
      filename: self.config.get('winston.file')
    }))
  }
  ...
  self.sysLog.info('Winston logger now active, will continue to sysLog init messages')
